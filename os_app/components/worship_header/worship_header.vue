<template>
	<view class="worship_header" :style="'background-image:url(' + bgImg + ')'">
		<!-- 顶上按钮 -->
		<view class="top_btn">
			<view class="top_btn_yy"><image :src="YY" :class="is_play ? 'active_yy' : ''" @click="is_music" /></view>
			<view class="top_btn_num">
				<image src="../../static/images/fx2.png" />
				<view class="fx_num">12312</view>
			</view>
		</view>
		...
		<!-- 头像 -->
		<image src="../../static/images/header.png" mode="aspectFill" class="icon" :style="{top:imgTop+'rpx'}"/>

		<!-- 信息 -->
		<view class="info" :style="{top:infotop+'rpx'}">
			<view class="name">撒娇哈佛</view>
			<view class="sex">男</view>
			<view class="time">1990-1990</view>
		</view>
	</view>
</template>

<script>
import { imaga64 } from '../../common/base64.js';
...
export default {
	...
	methods: {
		is_music() {
			this.YY == yy ? (this.YY = jyy) : (this.YY = yy);
